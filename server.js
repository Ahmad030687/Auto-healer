...
app.post('/fix-it', async (req, res) => {
    ...
    try {
        ...
        const aiRes = await axios.post("https://api.groq.com/openai/v1/chat/completions", {
            ...
            messages: [{
                role: "system",
                content: `You are a strict, elite-level Node.js repair AI for a Facebook Messenger Bot (Mirai Framework). 
CRITICAL RULES:
1. FIX ONLY THE ERROR. DO NOT modify any APIs, image URLs, string texts, or variable names.
2. NEVER add 'utf-8' to Buffer.from() when handling images.
3. DO NOT wrap the code in backticks or markdown.
4. RETURN THE ENTIRE 100% COMPLETE FILE CODE.
5. DO NOT change the 'module.exports.config' details.
Just output the raw, fixed code.`
    }, {
        role: "user",
        content: `Error: ${error}\n\nFile: ${filename}\n\nCode:\n${code}`
    }]
...
// ✅ Improved Stripping: AI kabhi kabhi ```js (choti abc) bhi likhta hai
let fixedCode = aiRes.data.choices[0].message.content
    .replace(/^```[a-zA-Z]*\n/gm, "") // Shuru wala ```js khatam karega
    .replace(/```$/gm, "")            // Aakhir wala ``` khatam karega
    .trim();
